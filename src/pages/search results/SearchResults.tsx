    ...
    <div>
        ...
        <div>
            ...
            {searchResults.artists.items.map((artist: { [key: string]: any })=>{
                return <Link to={"/artist/" + artist.id} key={artist.id}>{artist.name}</Link>
            })}
            </div>
...
export async function searchResultsLoader({ params }: { [key: string]: any }){
    ...
    let searchResults: { [key: string]: any } = {};
        ...
        let isError = false
        await fetchWebApi(`v1/search?q=${params.queryTerm}&type=track,artist,album`, "GET", token).then((res) => {
            if (res.error) {
